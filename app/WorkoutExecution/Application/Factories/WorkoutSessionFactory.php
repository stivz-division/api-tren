...
final readonly class WorkoutSessionFactory
{
    public function create(
        ...
        $exercises = array_map(
            fn (PlannedExerciseSnapshotData $exercise): WorkoutExercise => WorkoutExercise::fromPlan(
    ...
    private function plannedSets(PlannedExerciseSnapshotData $exercise): WorkoutSetCollection
    {
        if ($exercise->sets === []) {
            throw new InvalidTrainingProgramSnapshot;
        }
        ...
        $sets = array_map(
            static fn (PlannedSetSnapshotData $set): WorkoutSet => new WorkoutSet(
